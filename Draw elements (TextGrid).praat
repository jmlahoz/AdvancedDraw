...
if draw_textgrid and data_type$ = "TextGrid"
editor 'data_type$' 'data_name$'
tg_info$ = TextGrid info
n_int_tiers = extractNumber(tg_info$,"Number of interval tiers: ")
n_point_tiers = extractNumber(tg_info$,"Number of point tiers: ")
n_tiers = n_int_tiers + n_point_tiers
endeditor
outer_inch = 2.5 + (0.5*'n_tiers')
if n_tiers < 3
outer_inch = outer_inch + 1/8
endif
Select outer viewport... 0 6 0 'outer_inch'
Black
Line width... 1
editor 'data_type$' 'data_name$'
...
if draw_tier_names and draw_textgrid and data_type$ = "TextGrid"
Axes... 0.58 5.41 'outer_inch'-3/8 0.38
n = numberOfSelected()
for i from 1 to n
object'i' = selected(i)
endfor
...
endeditor
tier_width = (('outer_inch' - 3/8) - 2.25)/'n_tiers'
for tier_no from 1 to n_tiers
select 'tg'
tier_name$ = Get tier name... 'tier_no'
Text special... 5.45 left (2.25+('tier_width'/2)+(('tier_no'-1)*'tier_width')) half Times 8 0 'tier_name$'
endfor
endif
